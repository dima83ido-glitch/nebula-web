    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE) as db:
            await db.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (user[0], data["account_id"], data["name"], data["text1"], data["text2"],
                  data["text3"], int(data["interval"]), json.dumps(data["chats"])))
            await db.commit()
    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE) as db:
            cursor = await db.execute("""
                SELECT m.id, m.name, m.status, m.sent_count, a.phone 
                FROM mailings m JOIN accounts a ON m.account_id = a.id 
                WHERE m.owner_id=?
            """, (user[0],))
            rows = await cursor.fetchall()
        return json_response(True, mailings=[{"id":r[0],"name":r[1],"status":r[2],"sent":r[3],"phone":r[4]} for r in rows])
    except Exception as e:
        return json_response(False, str(e))
